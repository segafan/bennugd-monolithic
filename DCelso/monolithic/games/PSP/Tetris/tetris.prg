...
begin
	...
	tex2 = write_int(0,SCRW/2+100,210,5,&highscore[difficulty-1]);
	
	write(0,SCRW/2-50,190,3,"Difficulty:");
	text = write(0,SCRW/2+100,190,5,graden[difficulty-1]);
	...
	loop
		...
		if(checkButton(__LEFT) and difficulty>1)
			...
			delete_text(text);
			delete_text(tex2);
			text = write(0,SCRW/2+100,190,5,graden[difficulty-1]); //ERROR ON PSP
			tex2 = write_int(0,SCRW/2+100,210,5,&highscore[difficulty-1]);			
