...
{
    cpShape *res =cpSpacePointQueryFirst(cpEspacio, *(cpVect *)params[0], params[1], params[2]);
    return (int) res;
}
...
static int modcpSpaceSegmentQueryFirst(INSTANCE * my, int * params)
{
    cpVect *a=params[0],*b=params[1];
    cpSegmentQueryInfo info;
    cpShape *res = cpSpaceSegmentQueryFirst(cpEspacio,*a,*b,params[2],params[3],&info);
...
static int modSpaceUseSpatialHash(INSTANCE * my, int * params){
    cpSpaceUseSpatialHash (cpEspacio,*(float *)&params[1],params[1]);
    espacioHash=1;
    return 0;
}
...
static int modSpacePointQueryFirst(INSTANCE * my, int * params)
{
    ...
    cpShape * a=cpSpacePointQueryFirst(cpEspacio, cpv(*(float*)&params[0],*(float*)&params[1]), params[2], params[3]);
    int id=0;
    if (a!=NULL)
    {
        if (a->body->data!=NULL)
...
static int modSpaceSegmentQueryFirst(INSTANCE * my, int * params)
{
    ...
    cpSegmentQueryInfo info;
    int id=-1;
    cpShape *a = cpSpaceSegmentQueryFirst(cpEspacio,cpv(*(float*)&params[0],*(float*)&params[1]),cpv(*(float*)&params[2],*(float*)&params[3]),lay,group,&info);
    if (a!=NULL)
    {
...
static int modcrearHandler(INSTANCE * my, int * params)
{
    ...
    par[0]=params[0];
    par[1]=params[1];
    arregloItera(HandlerColisions, buscaABCol, &par);

    if (!Crear)
        return 0;
    cpArray* arr=nuevoArreglo(10);
    cpArray* arr2=nuevoArreglo(10);
    ...
    ArregloPush(HandlerColisions, col);
    cpSpaceAddCollisionHandler(cpEspacio, params[0], params[1], NULL, NULL, recogeColisionHandler, NULL, col);
    return (int) col;
}
...
static int modremoveHandler(INSTANCE * my, int * params)
{
    ...
    cpSpaceRemoveCollisionHandler(cpEspacio, col->a, col->b);
    cpArrayDeleteObj(HandlerColisions, col);
   // arregloItera(col->arr, (cpArrayIter)eliminaContactPointSetM, NULL);
    destruyeArreglo(&col->arr,1);
    //arregloItera(col->colisiones, (cpArrayIter)eliminaContactPointSetM, NULL);
    destruyeArreglo(&col->colisiones,1);
    free(col);
    return 0;
}
...
static int modgetColInfo(INSTANCE * my, int * params)
{
    ...
    if (ArregloCantidad(col->colisiones)>0)
    {
        cpContactPointSetM * set =(cpContactPointSetM *)ArregloPop(col->colisiones);
...
static int modaddInanimateShape(INSTANCE * my, int * params)
{
    if (params[0]<=TYPE_NONE || params[0]>=TYPE_EMPTY)
        return 0;
    cpBody * bod=cpEspacio->staticBody;
    cpShape * sha;
    ...
    LLagrega(ListaEstaticosFondo, sha);
    return (int) sha;
}

void modChipmunkEliminaEstatico(void * parametro){
    cpSpaceRemoveShape(cpEspacio,parametro);
    cpShapeFree(parametro);
}

int modChipmunkRemoveInanimateShape(INSTANCE * my, int * params){
    LLelimina(ListaEstaticosFondo,params[0], modChipmunkCompara, modChipmunkEliminaEstatico,1);
...
//                if (!estatico)
//                    cpSpaceRemoveBody(cpEspacio, elem->body);
//            }
//            else
//                estatico=1;
...
//                if (estatico)
//                {
//                    cpSpaceRemoveStaticShape(cpEspacio, (cpShape *)elem->Shapes[z]);
//                }
//                else
//                    cpSpaceRemoveShape(cpEspacio, (cpShape *)elem->Shapes[z]);
//            }
//
//            if (elem->body)
...
//        lista[numLista]=a;
//        cpSpaceAddPostStepCallback(cpEspacio, (cpPostStepFunc)postStepRemove, NULL, NULL);
//    }else{
//        modInitLista();
//        modAddLista(a);
//    }
...
//{
//    // cpSpaceAddPostStepCallback(cpEspacio, (cpPostStepFunc)postStepRemove, NULL, NULL);
//    //cpSpaceStep(cpEspacio, *(float *)GLOADDR(mod_chipmunk,GLO_INTERVAL));
...
{
    DataPointer aux;
    for (aux= ListaCuerpos; aux->sig!=NULL;)
    {
...
void actualizaProcess(DataPointer dat)
{
    ...
    if (gEstatico){
            cpSpaceReindexShapesForBody(cpEspacio,(cpBody *)LOCDWORD( mod_chipmunk, ins, LOC_BODY ));
    }
}
...
{
    cpEspacio->gravity.x= *(float *)GLOADDR(mod_chipmunk,GLO_GRAVITY_X);
    cpEspacio->gravity.y= *(float *)GLOADDR(mod_chipmunk,GLO_GRAVITY_Y);
    cpEspacio->collisionBias=*(float *)GLOADDR(mod_chipmunk,GLO_BIAS_COEF);
    cpEspacio->collisionSlop=*(float *)GLOADDR(mod_chipmunk,GLO_COLLISION_SLOP);
    cpEspacio->collisionPersistence=GLODWORD(mod_chipmunk,GLO_CONTACT_PERSISTENCE);
    cpEspacio->iterations= GLODWORD(mod_chipmunk,GLO_ITERATIONS);
    cpEspacio->damping= *(float *)GLOADDR(mod_chipmunk,GLO_DAMPING);
    cpEspacio->idleSpeedThreshold= *(float *)GLOADDR(mod_chipmunk,GLO_IDLESPEEDTHRESHOLD);
    cpEspacio->sleepTimeThreshold= *(float *)GLOADDR(mod_chipmunk,GLO_SLEEPTIMETHRESHOLD);
    for (contItUp=0; contItUp<GLODWORD(mod_chipmunk,GLO_PHRESOLUTION); contItUp++)
        cpSpaceStep(cpEspacio,*(float *)GLOADDR(mod_chipmunk,GLO_INTERVAL)/(float)GLODWORD(mod_chipmunk,GLO_PHRESOLUTION) );

    arregloItera(HandlerColisions,ActualizaLista,NULL);
...
{
    cpInitChipmunk();
    cpEspacio= (int)cpSpaceNew();
    inicializaListaProcesos();
    HandlerColisions=nuevoArreglo(10);
    LLinicializa(&ListaHandlers);
    LLinicializa(&ListaEstaticosFondo);
}
...
{
	cpArray *components = cpEspacio->sleepingComponents;
	while(components->num) cpBodyActivate((cpBody *)components->arr[0]);

	cpSpatialIndexEach(cpEspacio->staticShapes, (cpSpatialIndexIteratorFunc)&shapeFreeWrap, NULL);
	cpSpatialIndexEach(cpEspacio->activeShapes, (cpSpatialIndexIteratorFunc)&shapeFreeWrap, NULL);
	cpArrayFreeEach(cpEspacio->bodies, (void (*)(void*))cpBodyFree);
	cpArrayFreeEach(cpEspacio->constraints, (void (*)(void*))cpConstraintFree);
}
*/
...
{
    LLeliminaTodo(ListaHandlers,funcionElmHand,1);
    eliminaListaProcesos();
    LLeliminaTodo(ListaEstaticosFondo,cpShapeFree,1);
    //cpSpaceFreeChildren(cpEspacio);
    cpSpaceFree(cpEspacio);
 //   cpArrayEach(HandlerColisions,eliminaHandlerLista,NULL);
    destruyeArreglo(&HandlerColisions,1);
