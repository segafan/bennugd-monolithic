...
#define PRINT(x,y) stdout=fopen("log.txt","a");printf(x,y);fclose(stdout);
/* ---------------------------------------------------------------------- */
...
//    char * default_filename="game.dcb";
//    char * new_argv[2];
...
//#endif
    ...
    if ( standalone )
    {
    	/*INI DCELSO patch to do a default dcb file*/
        if ( argc<2 ){
               FILE *fd;
               fd = fopen(default_filename, "r");
               if (fd == NULL) {
