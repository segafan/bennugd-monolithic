...
begin
	...
	loop
		...
		elseif (mouseJoint<>0)
			RemoveConstraint(id,mouseJoint);
			mouseJoint = 0;
		end
		frame;
	end
...
begin
	...
	priority=father.priority;
	//esferaShape1 = addSegmentShape(0,15,0,-15,5);
...
begin
	lphysics.mass=5;
	lphysics.inertia=cpMomentForPoly(5.0, num, &verts, 0.0,0.0);
	priority=father.priority;
...
begin
	...
	loop
		...
		if (sub and key(_up) )
			...
			GetBody(lphysics.body,cp_c_v,&vel);
			//cpvslerp(&nPiso, cpv(0.0, -1.0,&aux), 0.75,&inter);
