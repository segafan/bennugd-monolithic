...
global
   res,db,i;
   filepath="score.db";
   SqlResult sqlRes;  // PORQUE ES UN STRUCT
   string fieldname[20];
...
begin
 ...
 if (db)
	...
	if (res==SQLITE_OK)	SQLITE3_EXEC(db,"COMMIT");
	else SQLITE3_EXEC(db,"ROLLBACK");	end


	write(0,100,130,0,"Records Affected : "+SQLITE3_TOTALCHANGES(db) ); //print records affected
  ...
  if (SqlRes.rows <= 0) write(0,100,150,0,"Not records found");
		if (1 <= 0) write(0,100,150,0,"Not records found");
		else write(0,100,150,0,"Press left and right to navigate..");
 ...
 while (not key(_esc))
    if (key(_left) )
	  if (SqlRes.CurrentRow>1) SqlRes.CurrentRow--;  mover=true; end;
	end;

    if (key(_right) )
	  if (SqlRes.CurrentRow < SqlRes.rows) SqlRes.CurrentRow++; mover=true; end;
	end;
	 ...
	 while (key(_right) or key(_left) ) frame; end; //clean keyboard buffer to navigate slow
	end;
