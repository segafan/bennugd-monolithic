 ...
 int modaddDampedSpring(INSTANCE * my, int * params)
{
    ...
    {
        b2 = cpEspacio->staticBody;
    }
    if (cpBodyIsStatic(b1) && cpBodyIsStatic(b2))
        return 0;
    ...
    cpSpaceAddConstraint(cpEspacio, z);
    addConstr(b1,b2,z);
    agregaData
    return (int)z;
}
...
void eliminaConstraint(void * par){
    ...
    nodo * n;
//    printf("inseer %d\n",c); fflush(stdout);
        if (cpConstraintGetA(c)!=cpEspacio->staticBody){    //el cuerpo estatico del fondo no almacena los constraints con los que interactúa, así que si el cuerpo del constraint es el fondo, que no intente borrar de él su registro
...
if (cpConstraintGetB(c)!=cpEspacio->staticBody){    //el cuerpo estatico del fondo no almacena los constraints con los que interactúa, así que si el cuerpo del constraint es el fondo, que no intente borrar de él su registro
    ...
    cpSpaceRemoveConstraint(cpEspacio, c);
    cpConstraintFree(c);
}
